...
resource "azurerm_policy_assignment" "baseline" {
  count                = var.assign_security_policies ? 1 : 0
  name                 = "${var.org_name}-security-assignment"
  scope                = local.scope
  policy_definition_id = var.policy_id
  description          = "${var.org_name} Baseline Security Policy Assignment"
  display_name         = "${var.org_name} Baseline Security Policy Assignment"
