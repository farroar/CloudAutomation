...
resource "azurerm_virtual_machine" "primary_fgt_vm" {
  count                        = var.fgt_ha_fabric_conn ? 1 : 0
  name                         = "primaryfgt"
  location                     = var.location
  resource_group_name          = var.fgt_rg_name
  network_interface_ids        = [azurerm_network_interface.primary_port1.id, azurerm_network_interface.primary_port2.id, azurerm_network_interface.primary_port3.id, azurerm_network_interface.primary_port4.id]
  primary_network_interface_id = azurerm_network_interface.primary_port1.id
  vm_size                      = var.size
  ...
  os_profile {
    ...
    custom_data    = data.template_file.primaryFortiGate.rendered
  }
...
data "template_file" "primaryFortiGate" {
  template = file(var.bootstrap-primary-sdn)
  vars = {
    ...
    port3_ip        = cidrhost(var.ha_subnet, 4)
    port3_mask      = cidrnetmask(var.ha_subnet)
    port4_ip        = cidrhost(var.mgmt_subnet, 4)
    port4_mask      = cidrnetmask(var.mgmt_subnet)
    passive_peerip  = cidrhost(var.ha_subnet, 5)
