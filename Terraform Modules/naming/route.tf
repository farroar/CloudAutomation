...
locals {
  # alphanum 1-80 with hypen, underscore and period.
    route = {
      ...
      standard  = (var.type == "route" && var.convention == "standard") ? substr("${local.filteredorgname.alphanumhup}-${var.location}-${local.filteredsubscription.alphanumhup}-${local.filtered.alphanumhup}${local.filteredpostfix.alphanumhup}${lookup(var.az_postfix, var.type)}", 0, local.max) : null 
