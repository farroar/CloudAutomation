...
locals {
  # Alphanumeric, period, hyphen, underscore.
    afw = {
      ...
      gpcxstandard  = (var.type == "afw" && var.convention == "gpcxstandard") ? substr("${local.filteredorgname.alphanumhup}-${var.location}-${local.filteredsubscription.alphanumhup}${local.filteredpostfix.alphanumhup}${lookup(var.az_postfix, var.type)}", 0, local.max) : null 
