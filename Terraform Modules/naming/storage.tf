...
locals {
  # Alphanumeric, period, hyphen, underscore.
    stor = {
      ...
      gpcxstandard  = (var.type == "stor" && var.convention == "gpcxstandard") ? substr("${local.filteredsubscription.alphanum}${local.filteredname.alphanum}${local.threedigitrandom_number}${lookup(var.az_postfix, var.type)}", 0, local.max) : null 
