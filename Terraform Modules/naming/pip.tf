...
locals {
  # alphanum 1-80 with hypen, underscore and period.
    pip = {
      ...
      standard  = (var.type == "pip" && var.convention == "standard") ? substr("${local.filteredorgname.alphanumhup}-${var.location}-${local.filtered.alphanumhu}${local.filteredpostfix.alphanumhup}${lookup(var.az_postfix, var.type)}", 0, local.max) : null 
