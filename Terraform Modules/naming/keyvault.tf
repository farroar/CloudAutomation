...
locals {
    ...
    kv = {
        ...
        gpcxstandard  = (var.type == "kv" && var.convention == "gpcxstandard") ? substr("${local.filteredorgname.alphanumhup}-${local.filteredsubscription.alphanumhup}-${local.filtered.alphanumhup}${local.filteredpostfix.alphanumhup}${lookup(var.az_postfix, var.type)}", 0, local.max) : null 
